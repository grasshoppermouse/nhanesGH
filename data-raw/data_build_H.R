...
#
# [1] "HSQ470"           "HSQ480"           "HSQ490"
# [4] "HSQ493"           "HSQ496"           "days_poor_health"
# [7] "T4free"           "TSH"              "MCQ051"
# [10] "MCQ140"

## THYROID DATA NOT COLLECTED THIS PHASE

H_data <- function(){
# Demographics ------------------------------------------------------------

dem = read.xport('data-raw/NHANES data/DEMO_H.XPT')
...
phq = read.xport('data-raw/NHANES data/DPQ_H.XPT')
...
hsq = read.xport('data-raw/NHANES data/HSQ_H.XPT')
hsq[hsq==7 | hsq==9 | hsq==77 | hsq==99] = NA

hsq$general_health = ordered(hsq$HSD010)
# hsq$days_poor_health = hsq$HSQ470 ### not collected this phase
...
pfq = read.xport('data-raw/NHANES data/PFQ_H.XPT')
...
pfq20 <- pfq[pfq$RIDAGEYR>=20 & !is.na(pfq$PFQ054),] # Missing data for age < 20
...
bmx = read.xport('data-raw/NHANES data/BMX_H.XPT')
...
# thyrod = read.xport('data-raw/NHANES data/THYROD_H.XPT')
...
whq = read.xport('data-raw/NHANES data/WHQ_H.XPT')
...
cbc = read.xport('data-raw/NHANES data/CBC_H.XPT')
...
mcq = read.xport('data-raw/NHANES data/MCQ_H.XPT')
...
diq = read.xport('data-raw/NHANES data/DIQ_H.XPT')
...
fsq <- read.xport('data-raw/NHANES data/FSQ_H.XPT')

# Health care -------------------------------------------------------------

# HIQ011 - Are you covered by some health care plan

hiq <- read.xport('data-raw/NHANES data/HIQ_H.XPT')
...
d$depressed_int <- as.integer(d$depressed)
d$perceived_abnormal_weight_int <- as.integer(d$perceived_abnormal_weight)
d$special_equipment_int <- as.integer(d$special_equipment)

d_H <- d
# save(d_H, file='d_H.rda')
return(d_H)
}
